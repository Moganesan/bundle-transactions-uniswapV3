  ...
  private calculateBundlePricing(
    ...
    const bundleGasPricing = bundleTransactions.reduce(
      (acc, transactionDetail) => {
        ...
        const priorityFeeReceivedByMiner =
          totalMinerReward.sub(ethSentToCoinbase);
        return {
          gasUsed: acc.gasUsed + gasUsed,
          gasFeesPaidBySearcher: acc.gasFeesPaidBySearcher.add(
            baseFee.mul(gasUsed).add(priorityFeeReceivedByMiner)
          ),
          priorityFeesReceivedByMiner: acc.priorityFeesReceivedByMiner.add(
            priorityFeeReceivedByMiner
          ),
    ...
    const effectiveGasPriceToSearcher =
      bundleGasPricing.gasUsed > 0
        ? bundleGasPricing.ethSentToCoinbase
            .add(bundleGasPricing.gasFeesPaidBySearcher)
